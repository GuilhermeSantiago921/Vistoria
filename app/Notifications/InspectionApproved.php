...
class InspectionApproved extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
                    ->subject('✅ Vistoria APROVADA - Sistema de Vistoria')
                    ->greeting('Parabéns ' . $notifiable->name . '!')
                    ->line('Sua vistoria foi **APROVADA** por nossa equipe técnica!')
                    ->line('')
                    ...
                    ->line('Você já pode baixar o relatório completo da vistoria.')
                    ->action('Baixar Relatório', route('report.pdf', $this->inspection->id))
